...
MapLayer::MapLayer(Shader &shader, Texture2D &texture, tmx_map *map, tmx_layer *layer){
    unsigned long i, j;
    ...
    for (i=0; i<map->height; i++) {
        for (j=0; j<map->width; j++) {
            ...
            if (map->tiles[gid] != NULL) {
                ...
                float vert_tl_x,vert_tl_y;
                float vert_bl_x,vert_bl_y;
                float vert_br_x,vert_br_y;
                float vert_tr_x,vert_tr_y;
                
                vert_tl_x = dx/this->map_width;
                vert_tl_y = (dy + w)/this->map_height;
                
                vert_bl_x = dx/this->map_width;
                vert_bl_y = dy/this->map_height;
                
                vert_br_x = (dx + w)/this->map_width;
                vert_br_y = dy/this->map_height;
                
                vert_tr_x = (dx + w)/this->map_width;
                vert_tr_y = (dy + h)/this->map_height;
                ...
                tex_tl_x = x/texWidth;
                tex_tl_y = (y + h)/texHeight;
                
                tex_bl_x = x/texWidth;
                tex_bl_y = y/texHeight;
                
                tex_br_x = (x + w)/texWidth;
                tex_br_y = y/texHeight;
                
                tex_tr_x = (x + w)/texWidth;
                tex_tr_y = (y + h)/texHeight;
